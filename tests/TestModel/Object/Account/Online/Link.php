...
/**
 * Odoo model : account.online.link
 * ---
 * Name : account.online.link
 * ---
 * Info :
...
final class Link extends Base
{
     ...
     * ---
     * Relation : one2many (account.online.account -> account_online_link_id)
     ...
     */
    private $account_online_account_ids;
     ...
     * Provider Data
     * ---
     * Information needed to interract with third party provider
     * ---
     * Searchable : yes
     * Sortable : yes
     *
     ...
     *        Sortable : yes
     */
    public function __construct(string $state, OdooRelation $company_id)
    {
        $this->state = $state;
     ...
     * @return OdooRelation[]|null
     *
     * @SerializedName("account_online_account_ids")
     */
    public function getAccountOnlineAccountIds(): ?array
    {
        return $this->account_online_account_ids;
    }
    ...
    /**
     * @param bool|null $auto_sync
     */
    public function setAutoSync(?bool $auto_sync): void
    {
     ...
     * @param OdooRelation $item
     */
    public function removeAccountOnlineAccountIds(OdooRelation $item): void
    {
        if (null === $this->account_online_account_ids) {
            $this->account_online_account_ids = [];
        }

        if ($this->hasAccountOnlineAccountIds($item)) {
            $index = array_search($item, $this->account_online_account_ids);
            unset($this->account_online_account_ids[$index]);
        }
    }
     ...
     * @param OdooRelation $item
     */
    public function addAccountOnlineAccountIds(OdooRelation $item): void
    {
        if ($this->hasAccountOnlineAccountIds($item)) {
            return;
        }
        ...
        $this->account_online_account_ids[] = $item;
    }
     ...
     * @return bool
     */
    public function hasAccountOnlineAccountIds(OdooRelation $item): bool
    {
        if (null === $this->account_online_account_ids) {
            return false;
        }

        return in_array($item, $this->account_online_account_ids);
    }

    /**
     * @param OdooRelation[]|null $account_online_account_ids
     */
    public function setAccountOnlineAccountIds(?array $account_online_account_ids): void
    {
        $this->account_online_account_ids = $account_online_account_ids;
    }
